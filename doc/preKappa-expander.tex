...
\begin{frame}
  ...
  \begin{flushleft}
    ...
    \item Freely available on \structure <4>{https://github.com/ajendrex/expander/}
  \end{flushleft}
\end{frame}
...
\begin{frame}
  ...
  \begin{flushleft} {\small
    ...
    \item \structure <1>{\%expand-init: all ADensity} A(x,c) \newline
    \structure <1>{\%expand-init: all BDensity} B(x) \pause
...
\begin{frame}
  ...
  \begin{flushleft} {\scriptsize
    ...
    \structure <2>{\%expand-rule: TM} A(x,c),T() $\rightarrow$ A(\structure <2>{\%},x,c),T() @ 1 \pause
    \item \structure <3>{gives:} \newline
